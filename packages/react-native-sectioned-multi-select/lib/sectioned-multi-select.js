...
class SectionedMultiSelect extends PureComponent {
  ...
  _filterItems = (searchTerm) => {
    ...
    items && items.forEach((item) => {
      ...
      if (regex.test(this.getProp(item, findKey))) {
      ...
      if (item[subKey]) {
        ...
        item[subKey].forEach((sub) => {
          if (regex.test(this.getProp(sub, findKey))) {
  ...
  _renderChips = () => {
    ...
    return (
      selectedItems.length > 0 && !single && showChips && !customChipsRenderer ?
        <View
          style={[{
            flexWrap: 'wrap',
            alignItems: 'center',
